...
namespace VehicleServiceManagement.Controllers
{
    ...
    public class ServiceAdvisorController : ControllerBase
    {
        ...
        [HttpPost("materialBills")]
        public async Task<ActionResult<MaterialBill>> AddMaterialBill([FromBody] MaterialBill request)
        {
        ...
        [HttpPost("complete-service-record")]
        public async Task<ActionResult> CompleteServiceRecord(int vehicleId)
        {
            // Find the service record
            var serviceRecord = await _context.ServiceRecords
        .FirstOrDefaultAsync(sr => sr.VehicleId == vehicleId);
            ...
            return Ok( "Service record completed.");
        }
    }
}
